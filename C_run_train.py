...
class MainClass:
	...
	def run(self):
		...
		start = time.time()
		experiment = 0
		optList = ['Adamax','adam','SGD','Adagrad','Adadelta']
		for element in optList:
			
			startIteration = time.time()
			
			experiment += 1
			filename = 'C' + str(experiment) + '.h5'
			rl = RLsys(actions, size, optt = element)
			if self.loadNetwork:
			...
			for i in range(comRep.shape[2]):
				...
				for j in range(4):
					...
					print("Progress on " + "C" + str(experiment) +": " + str(100*(time.time() - startIteration)/(totalTime/runs)) +" %")
