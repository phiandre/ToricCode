...
class Env:
	...
	def __init__(self, compState, humanState=np.zeros(0), groundState=0, checkGroundState=False):
		...
		self.length = compState.shape[0]
		self.state =np.ones((self.length,self.length,3))
		
		# self.state[:,:,0] = compState
		# self.state[:,:,1] = pathHorizontal
		# self.state[:,:,2] = pathVertical
		self.state[:,:,0] = np.copy(compState)
	...
	def updateErrors(self):
		self.errors = np.transpose(np.nonzero(self.state[:,:,0]))
	...
	def moveError(self, action, errorIndex):
		...
		self.state[firstPos[0], firstPos[1],0] = 0
		# Uppdatera den nya positionen
		if self.state[secondPos[0], secondPos[1], 0] == 0:
			self.state[secondPos[0], secondPos[1], 0] = 1
		else:
			self.state[secondPos[0],secondPos[1], 0] = 0
		
		if action == 0:
			self.state[firstPos[0],firstPos[1],1] *= -1
		if action == 1:
			self.state[secondPos[0],secondPos[1],1] *= -1
		if action == 2:
			self.state[firstPos[0],firstPos[1],2] *= -1
		if action == 3:
			self.state[secondPos[0],secondPos[1],2] *= -1
	...
	def getObservation(self):
		if len(self.errors)==0:
			...
			observation=np.zeros((self.length,self.length,3,numerror))
