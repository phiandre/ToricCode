import numpy as np
from Env_mem import Env
from RL_mem import RLsys
...
class MainClass:
	...
	def __init__(self):
		...
		"""
		Om du laddar in ett färdigt nätverk, kom då ihåg att kontrollera epsilonkoden
		"""
		
		# Groundstate Reward Increase Parameters
		finalReward = 5
		self.A = 0.5*finalReward
		self.B = self.A
		self.w = math.pi/110000
		self.b = -80000
	...
	def run(self):
		...
		for i in range(comRep.shape[2]):
			for j in range(4):
				...
				rl.epsilon = ((self.k+trainingIteration)/self.k)**(self.alpha) #(1+trainingIteration)**(self.alpha)
				
				env.cGS = self.A*np.tanh(self.w*(trainingIteration+self.b))+self.B
				env.wGS = -1
