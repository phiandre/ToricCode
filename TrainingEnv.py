...
class Env:
	...
	def moveError(self, action):
		...
		if self.state[secondPos[0], secondPos[1]] == 0:
			self.state[secondPos[0], secondPos[1]] = self.marker
		else:
	...
	def getObservation(self):
		...
		if len(x)==0:
			return 'terminal'
		centralizedState = self.centralize(np.array([int(x),int(y)]))
		if len(np.nonzero(centralizedState)) == 1:
			self.state[int(x), int(y)] = 1
			return 'terminal'
	...
	def markError(self):
		errorIndex = np.random.randint(0, len(self.getErrors()))
		coords = self.getErrors()[errorIndex,:]
		x = coords[0]
		y = coords[1]
