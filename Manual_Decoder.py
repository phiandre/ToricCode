...
class JukeBox:
	...
	def wrongSound(self):
		...
		cMajor =[int(A/2),int(B/2),C,D,E,F,G,A,B,C_]
		melodyNotes = list()
		melodyBeat = list()
		
		b = 300
	
		melodyPrep = [2,0,3,2,0,2,2,0,3,2,0]
		
		for i in range(11):
			melodyNotes.append(cMajor[melodyPrep[i]])

		
		melodyBeat.append(2*b)
		melodyBeat.append(b)
		melodyBeat.append(b)
		melodyBeat.append(2*b)
		melodyBeat.append(2*b)
		...
		for i in range(len(melodyNotes)):
			winsound.Beep(melodyNotes[i],melodyBeat[i])
		
		def correctSound(self):
		C = 523
		D = 587
		E = 659
		F = 698
		...
		melodyNotes.append(cMajor[2])
		melodyNotes.append(cMajor[4])
		melodyNotes.append(cMajor[6])
		melodyNotes.append(cMajor[9])
		melodyNotes.append(cMajor[6])
		melodyNotes.append(cMajor[9])
	...
	def run(self):
		...
		for i in range(min(comRep.shape[2],self.maxNumberOfIterations)):
			...
			if r == env.correctGsR:
				...
				song = thr.Thread(target = self.correctSound())
				song.start()
			else:
			
				print("Topological Ground State is incorrect!")
				song = thr.Thread(target = self.wrongSound())
				song.start()
